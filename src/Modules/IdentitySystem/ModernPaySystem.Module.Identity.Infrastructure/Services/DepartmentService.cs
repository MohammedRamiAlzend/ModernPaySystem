    ...
    ILogger<DepartmentService> logger) : IDepartmentService
{
    public async Task<Result<DepartmentDto>> CreateAsync(CreateDepartmentDto dto, string userId)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(dto.Name))
                return new Error("InvalidInput", "Department name is required.", ErrorKind.Validation);
            ...
            var userResult = await unitOfWork.Users.GetAsync(
                x => x.Id == dto.HeadedUserId,
                i => i.Include(x => x.HeadedDepartment));
            if (userResult.IsError)
                return userResult.Errors;
            if (userResult.Value!.HeadedDepartmentId.HasValue)
            {
                return new Error("USER_ALREADY_HEAD",
                    $"User is already head of department {userResult.Value.HeadedDepartment.Name}",
                    ErrorKind.Conflict);
            }
        ...
        catch (Exception ex)
        {
            ...
            return Error.Failure("InternalServerError", "An error occurred while creating the department.");
        }
    }
    ...
    public async Task<Result<DepartmentDto>> UpdateAsync(Guid id, UpdateDepartmentDto dto, string userId)
    {
        ...
        catch (Exception ex)
        {
            ...
            return Error.Failure("InternalServerError", "An error occurred while updating the department.");
        }
    }
    ...
    public async Task<Result<bool>> DeleteAsync(Guid id)
    {
        try
        {
            bool hasChildren = await unitOfWork.Departments.AnyAsync(d => d.ParentDepartmentId == id);
            if (hasChildren)
        ...
        catch (Exception ex)
        {
            ...
            return Error.Failure("InternalServerError", "An error occurred while deleting the department.");
        }
    }
    ...
    public async Task<Result<List<DepartmentTreeDto>>> GetTreeAsync()
    {
        try
        {
            var roots = await unitOfWork.Departments.GetAllAsync(
                d => d.ParentDepartmentId == null,
                i => i.Include(x => x.DepartmentHead).Include(x => x.ChildDepartments));
            if (roots.IsError)
                return roots.Errors;
        ...
        catch (Exception ex)
        {
            ...
            return Error.Failure("InternalServerError", "An error occurred while fetching the department tree.");
        }
    }
    ...
    public async Task<Result<List<DepartmentTreeDto>>> GetSubTreeAsync(Guid departmentId)
    {
        try
        {
            var deptResult = await unitOfWork.Departments.GetAsync(
                x => x.Id == departmentId,
                i => i.Include(x => x.DepartmentHead).Include(x => x.ChildDepartments));
        ...
        catch (Exception ex)
        {
            ...
            return Error.Failure("InternalServerError", "An error occurred while fetching the department subtree.");
        }
    }
    ...
    public async Task<Result<List<DepartmentDto>>> GetChildrenAsync(Guid departmentId)
    {
        try
        {
            var children = await unitOfWork.Departments.GetAllAsync(
                d => d.ParentDepartmentId == departmentId,
                i => i.Include(x => x.DepartmentHead));
            if (children.IsError)
        ...
        catch (Exception ex)
        {
            ...
            return Error.Failure("InternalServerError", "An error occurred while fetching department children.");
        }
    }
    ...
    public async Task<Result<DepartmentDto?>> GetParentAsync(Guid departmentId)
    {
        ...
        catch (Exception ex)
        {
            ...
            return Error.Failure("InternalServerError", "An error occurred while fetching the parent department.");
        }
    }
    ...
    public async Task<Result<List<DepartmentDto>>> SearchAsync(string? searchTerm = null, int level = 0)
    {
        try
        {
            ...
            {
                filters.Add(d => d.Name.Contains(searchTerm) || (d.Code != null && d.Code.Contains(searchTerm)));
            }
        ...
        catch (Exception ex)
        {
            ...
            return Error.Failure("InternalServerError", "An error occurred while searching departments.");
        }
    }
    ...
    public async Task<Result<List<DepartmentDto>>> GetByLevelAsync(int level)
    {
        try
        {
            ...
            return allDepts.Value!.Select(MapToDto).ToList();
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error fetching departments at level: {Level}", level);
            return Error.Failure("InternalServerError", "An error occurred while fetching departments.");
        }
    }
    ...
    public async Task<Result<List<DepartmentDto>>> GetPathToRootAsync(Guid departmentId)
    {
        try
        {
            var path = new List<DepartmentDto>();
            ...
            while (currentId != Guid.Empty && depth < maxDepth)
            {
                ...
                path.Add(MapToDto(result.Value));
        ...
        catch (Exception ex)
        {
            ...
            return Error.Failure("InternalServerError", "An error occurred while fetching the department path.");
        }
    }
    ...
    public async Task<Result<List<UserDto>>> GetUsersInDepartmentAsync(Guid departmentId, bool includeSubDepartments = false)
    {
        try
        {
            var deptResult = await unitOfWork.Departments.GetAsync(
                x => x.Id == departmentId,
                i => i.Include(x => x.Users).ThenInclude(x => x.SubSystemUser));
            if (deptResult.IsError || deptResult.Value == null)
                return new Error("NOT_FOUND", "Department not found", ErrorKind.NotFound);
        ...
        catch (Exception ex)
        {
            ...
            return Error.Failure("InternalServerError", "An error occurred while fetching department users.");
        }
    }
    ...
    public async Task<Result<bool>> AssignUserToDepartmentAsync(Guid userId, Guid departmentId)
    {
        try
        {
            ...
            logger.LogInformation("Assigned user: {UserId} to department: {DepartmentId}", userId, departmentId);
            return true;
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error assigning user to department");
            return Error.Failure("InternalServerError", "An error occurred while assigning user to department.");
        }
    }
    ...
    public async Task<Result<bool>> AssignDepartmentHeadAsync(Guid departmentId, Guid userId)
    {
        try
        {
            ...
            if (user.IsDepartmentHead && user.HeadedDepartmentId.HasValue && user.HeadedDepartmentId.Value != departmentId)
                return new Error("USER_ALREADY_DEPARTMENT_HEAD",
                    "User is already assigned as head of another department", ErrorKind.Validation);
        ...
        {
            logger.LogError(ex, "Error assigning department head");
            return Error.Failure("InternalServerError", "An error occurred while assigning department head.");
        }
    }
    ...
    public async Task<Result<bool>> RemoveUserFromDepartmentAsync(Guid userId)
    {
        ...
        catch (Exception ex)
        {
            ...
            return Error.Failure("InternalServerError", "An error occurred while removing user from department.");
        }
    }
    ...
    private static DepartmentTreeDto MapToTreeDto(Department department)
    {
