...
import type { TransactionPeriodReport } from '../../model/transaction-report-types';
...
interface PeriodReportViewProps {
    data: TransactionPeriodReport | undefined;
    isLoading: boolean;
    periodLabel: string;
}
...
function StatCard({ label, value }: { label: string; value?: number | null }) {
    return (
        <div className="rounded-lg border border-border/40 bg-card p-3 text-center shadow-sm">
            <div className="text-2xl font-bold text-foreground">{(value ?? 0).toLocaleString()}</div>
            ...
            <Card className="border border-border/40 shadow-sm">
                ...
                <CardContent>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <StatCard label="طلبات منشأة" value={data.totalRequestsCreated} />
                        <StatCard label="ردود مضافة" value={data.totalResponsesMade} />
                        <StatCard label="مرفقات مضافة" value={data.totalAttachmentsAdded} />
