...
interface DeletionRequestsListProps {
    requests: DeleteArchiveRequest[];
    isLoading: boolean;
    onViewDetails: (request: DeleteArchiveRequest) => void;
}

export function DeletionRequestsList({ requests, isLoading, onViewDetails }: DeletionRequestsListProps) {
            ...
            { id, notes },
            {
                onSuccess: () => {
                    showStatus({
                        type: 'success',
                        title: 'تمت الموافقة',
                        message: 'تمت الموافقة على طلب الحذف وتم حذف المستند/المجلد.'
                    });
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center p-12 gap-3 text-muted-foreground">
                ...
                <span className="text-xs font-bold">جاري تحميل طلبات الحذف...</span>
            </div>
        );
    }
    ...
    if (requests.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center p-16 gap-3 text-muted-foreground border border-dashed border-border rounded-3xl bg-muted/5">
                ...
                <span className="text-xs font-bold">لا توجد طلبات حذف معلقة حالياً</span>
            </div>
        );
    }
                ...
                <table className="w-full text-sm text-right text-foreground">
                    <thead className="text-xs text-muted-foreground font-bold bg-muted/30 border-b border-border">
                        <tr>
                            ...
                            <th className="px-6 py-4">سبب طلب الحذف</th>
                                ...
                                <tr className="hover:bg-muted/10 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2 justify-end">
                                            {req.targetType === ArchiveDeletionTargetType.Record ? (
                                                <FileText className="h-4 w-4 text-muted-foreground" />
                                            ) : (
                                                <Folder className="h-4 w-4 text-muted-foreground" />
                                            )}
                                            <span className="font-bold text-foreground">
                                                {req.targetType === ArchiveDeletionTargetType.Record ? 'مستند' : 'مجلد'}
                                            </span>
                                        </div>
                                    </td>
                                        ...
                                        <td colSpan={5} className="bg-muted/10 p-4 border-t border-border">
                                            <div className="flex flex-col gap-3 max-w-lg mr-auto text-right">
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    >
                                                        {actionType === 'approve' ? 'تأكيد الموافقة والحذف' : 'تأكيد الرفض'}
                                                    </Button>
                                                    <Input
